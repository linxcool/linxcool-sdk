...
public class LaunchPluginVerifier {
	...
	public static LaunchPluginInfo verify(InputStream is){
		return verify(new DataInputStream(is));
	}

	public static LaunchPluginInfo verify(DataInputStream dis){
		try{
			...
			comment = LaunchSecurityUtil.mixDecrypt(comment);

			String[] data = comment.split("\\|");
			if(data.length < 3)
				return null;

			LaunchPluginInfo info = new LaunchPluginInfo();
