...
public class PluginInfoManager {
	 ...
	 * @param pluginInfo
	 */
	public static void setPluginHistory(Context context,PluginInfo pluginInfo){
	 ...
	 * @return
	 */
	public static PluginInfo getAssetsPluginInfo(Context context, String pluginName, String suffix){
		InputStream is = null;
		...
		// assets目录下插件的信息
		PluginInfo assetsInfo = null;
		try {
			if(is != null)assetsInfo = PluginVerifier.verify(is);
	 ...
	 * @return
	 */
	public static PluginInfo getLocalPluginInfo(Context context, String pluginName, String suffix){
		String fileName = getPluginHistory(context, pluginName);
		if(fileName == null)
			return getAhistoricalPluginInfo(context, pluginName, suffix);
		else{
			File pluginFile = ResourceUtil.newPluginFile(context, pluginName, fileName);
			...
			PluginInfo assetsInfo = getAssetsPluginInfo(context, pluginName, suffix);
			...
			pluginInfo.savePath = ResourceUtil.getPluginsFolder(context, pluginName);
	 ...
	 * @return
	 */
	private static PluginInfo getAhistoricalPluginInfo(Context context, String pluginName, String suffix){
		// assets目录下插件的信息
		PluginInfo assetsInfo = getAssetsPluginInfo(context, pluginName, suffix);
		// 检索插件目录下的插件信息
		String folder = ResourceUtil.getPluginsFolder(context, pluginName);
		PluginInfo localInfo = searchMaxVerPluginInfo(folder, pluginName, suffix);
		// 需要联网下载
	 ...
	 * @return 返回存储在本地的插件信息
	 */
	private static PluginInfo retrieveAssetsPlugin(
			Context context, PluginInfo assetsInfo, String suffix){
		
		String folder = ResourceUtil.getPluginsFolder(context, assetsInfo.plugName);
		...
		if(file.exists()){
			PluginInfo localInfo = null;
			try {
				localInfo = PluginVerifier.verify(file);
			} catch (Exception e) {
				e.printStackTrace();
				localInfo = null;
				file.delete();
		...
		if(ResourceUtil.retrieveFileFromAssets(context, assetFileName, filePath)){
	 ...
	 * @return 
	 */
	private static PluginInfo searchMaxVerPluginInfo(String folder, String pluginName, String suffix){
		...
		// 校验文件信息
		try {
			pluginInfo = PluginVerifier.verify(maxVerFile);
		} catch (Exception e) {
			e.printStackTrace();
			pluginInfo = null;
		}
